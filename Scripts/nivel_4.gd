extends Node2D
@export var tiempo_inicial: int = 43
...
func terminar_mision():
	await get_tree().create_timer(0.01).timeout  # ⏳ medio segundo de retraso
	get_tree().change_scene_to_file("res://Scenes/mision4.tscn")
